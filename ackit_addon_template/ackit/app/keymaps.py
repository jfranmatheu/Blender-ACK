...
from ..enums.event import EventType, EventValue
...
def late_register():
    ...
    for shortcut in operator_shortcuts:
        ...
        modifiers = {}
        if mod_keys := shortcut.modifiers:
            for mod_key in mod_keys:
                modifiers[mod_key.lower()] = True
        if km is None:
            km = addon_km_config.keymaps.new(shortcut.keymap_idname, space_type=shortcut.space_type, region_type=shortcut.region_type)
        km.keymap_items.new(shortcut.operator.bl_idname, type=shortcut.event_type, value=shortcut.event_value, **modifiers)

        # print_debug("Register Shortcut", km.name, shortcut.operator.bl_idname)
